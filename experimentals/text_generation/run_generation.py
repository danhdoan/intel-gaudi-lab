...
def setup_parser(parser):
    ...
    if args.quant_config and args.load_quantized_model_with_autogptq:
        raise RuntimeError(
            "Setting both quant_config and load_quantized_model_with_autogptq is unsupported. "
        )
    if args.quant_config and args.load_quantized_model_with_autoawq:
        raise RuntimeError(
            "Setting both quant_config and load_quantized_model_with_autoawq is unsupported. "
        )
...
def main():
    ...
    if args.dataset_name is None:
        ...
        if args.prompt:
            ...
            input_sentences = assemble_prompt(
                prompt_size=args.max_input_tokens,
                book_path=download_book(book_ids[0]),
            )
        else:
            input_sentences = [
        ...
        if args.batch_size > len(input_sentences):
            ...
            for i in range(num_sentences_to_add):
                input_sentences.append(
                    input_sentences[i % len(input_sentences)]
                )
        ...
        def generate(size=None, reduce_recompile=False):
            ...
            if args.max_input_tokens > 0:
                if (
                    hasattr(model.config, "type_vocab_size")
                    and model.config.type_vocab_size > 0
                ):
                ...
                valid_sequence_lengths = compute_valid_sequence_lengths_tensor(
                    input_tokens
                ).to(args.device)
                generation_config.valid_sequence_lengths = (
                ...
                input_ids = tokenizer.apply_chat_template(
                    input_messages, return_tensors="pt", padding=True
                )
                ...
                input_tokens = tokenizer.batch_encode_plus(
                    input_sentences, return_tensors="pt", padding=True
                )
            ...
            if args.input_embeds:
                inputs_embeds = prepare_generation_embedding(
                    model, args.model_name_or_path, input_tokens
                )
            ...
            logger.info(f"Time to generate = {iteration_times[-1] * 1000}ms")
        ...
        if dyn_prompt_lens is None or len(set(dyn_prompt_lens)) == 1:
            for i in range(args.warmup):
                if dyn_prompt_lens is None:
                    print(
                        f"Warming up iteration {i + 1}/{args.warmup}",
                        flush=True,
                    )
            ...
            for i in range(args.warmup):
                lst = list(
                    range(
                        min_prompt_len, max_sentence_len + 1, args.bucket_size
                    )
                )
                for sz in lst:
                    print(
        ...
        for k, v in mem.items():
            print(
                "{:35} = {} GB".format(k[:-5].replace("_", " ").capitalize(), v)
            )
        print(
            f"Graph compilation duration          = {compilation_duration} seconds"
        )
        print(separator)
    else:
        ...
        assert (
            not args.simulate_dyn_prompt
        ), "Both dataset_name and simulate_dyn_prompt are set"
        ...
        raw_dataset = (
            ...
            .select(
                range(
                    args.dataset_max_samples
                    if args.dataset_max_samples > 0
                    else (raw_dataset[split]).num_rows
                )
            )
        )
        ...
        prompt_length = (
            ...
            if args.max_input_tokens > 0
            else (-1, 16)[args.max_input_tokens == 0]
        )
                    ...
                    result[k] = torch.stack(
                        [
                            torch.cat(
                                (
                                    torch.zeros(
                                        max_shape - t.shape[0], dtype=t.dtype
                                    ),
                                    t,
                                )
                            )
        ...
        stats = (
            f"Throughput (including tokenization) = {throughput} tokens/second"
        )
        separator = "-" * len(stats)
        print()
        print("Stats:")
    ...
    if args.const_serialization_path and os.path.isdir(
        args.const_serialization_path
    ):
        import shutil
