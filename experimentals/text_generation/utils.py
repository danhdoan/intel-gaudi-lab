...
def setup_const_serialization(const_serialization_path):
    ...
    const_serialization_path = os.path.join(
        const_serialization_path + uuid.uuid4().hex
    )
...
def setup_model(args, model_dtype, model_kwargs, logger):
    ...
    if args.use_hpu_graphs:
        ...
        if _is_peft_model(model):
            ...
            if model.peft_type == "ADAPTION_PROMPT":
                model.base_model.model = wrap_in_hpu_graph(
    ...
    if args.torch_compile:
        ...
        assert (
            "PT_HPU_LAZY_MODE" in os.environ
            and os.environ["PT_HPU_LAZY_MODE"] == "0"
...
def setup_distributed_model_tp(
    args, model_dtype, model_kwargs, logger, cache_dir
):
    ...
    model = AutoModelForCausalLM.from_config(
        config, torch_dtype=model_dtype, **model_kwargs
    )
...
def setup_distributed_model(args, model_dtype, model_kwargs, logger):
    ...
    if load_to_meta:
        ...
        checkpoints_json = tempfile.NamedTemporaryFile(
            suffix=".json", mode="+w"
        )
        ...
        if args.peft_model is not None:
            ...
            if args.local_rank == 0:
                ...
                peft_model(
                    args, model_dtype, logger, **model_kwargs
...
def peft_model(args, model_dtype, logger, **model_kwargs):
    ...
    if hasattr(model, "merge_and_unload"):
        ...
        if model.peft_type == "ADAPTION_PROMPT":
            ...
            tuners.adaption_prompt.layer.AdaptedAttention.pre_attn_forward = (
                GaudiAdaptedAttentionPreAttnForward
            )
            tuners.adaption_prompt.layer.AdaptedAttention.__getattr__ = (
                GaudiAdaptedAttention_getattr
            )
...
def setup_tokenizer(args, model, assistant_model, logger):
    ...
    if model.config.model_type == "llama":
        ...
        if assistant_model is not None:
            if assistant_model.generation_config.pad_token_id is None:
                if isinstance(
                    assistant_model.generation_config.eos_token_id, int
                ):
                    assistant_model.generation_config.pad_token_id = (
                        assistant_model.generation_config.eos_token_id
                    )
                elif isinstance(
                    assistant_model.generation_config.eos_token_id, list
                ):
                    assistant_model.generation_config.pad_token_id = (
                        assistant_model.generation_config.eos_token_id[0]
                    )
    ...
    if model.config.model_type == "minicpm3" and isinstance(
...
def setup_generation_config(args, model, assistant_model, tokenizer):
    ...
    generation_config.flash_attention_causal_mask = (
        args.flash_attention_causal_mask
    )
    generation_config.flash_attention_fast_softmax = (
        args.flash_attention_fast_softmax
    )
        ...
        if args.world_size == 2 or args.world_size == 4 or args.world_size == 8:
            if (
                ...
                if args.max_input_tokens >= 8192 and args.max_new_tokens >= 128:
                    return False
            elif args.max_input_tokens >= 4096 and args.max_new_tokens >= 128:
                return False
        return True
    else:
        return False
...
def initialize_model(args, logger):
    ...
    if not args.world_size > 0 and args.attn_batch_split > 1:
        logger.warning(
            "Disabling attention batch splitting as it's unnecessary for single-card execution"
        )
    ...
    setup_device(args)
    set_seed(args.seed)
    cache_dir = get_repo_root(
        args.model_name_or_path, local_rank=args.local_rank, token=args.token
    )
    if args.assistant_model is not None:
        get_repo_root(
    ...
    if (
        args.load_quantized_model_with_inc
        or args.local_quantized_inc_model_path
    ):
    ...
    model, assistant_model = (
        ...
        if not use_deepspeed or args.load_quantized_model_with_inc
            ...
            if args.parallel_strategy == "none"
                ...
                setup_distributed_model_tp(
                    args, model_dtype, model_kwargs, logger, cache_dir
                )
                if args.parallel_strategy == "tp"
                else setup_distributed_model_ep(
                    args, model_dtype, model_kwargs, logger
                )
            )
        )
    )

    tokenizer, model, assistant_model = setup_tokenizer(
        args, model, assistant_model, logger
    )
    generation_config = setup_generation_config(
        args, model, assistant_model, tokenizer
    )
        ...
        if name in gathered_state_dict:
            ...
            if len(param.shape) != 0 and full_weight.shape != param.shape:
                if full_weight.shape[0] != param.shape[0]:
                    split_weight = split_weights(
                        full_weight, world_size, local_rank, split_axis=0
                    ).clone()
                elif full_weight.shape[1] != param.shape[1]:
                    split_weight = split_weights(
                        full_weight, world_size, local_rank, split_axis=1
                    ).clone()
                else:
                    split_weight = split_weights(
                        full_weight, world_size, local_rank, split_axis=0
