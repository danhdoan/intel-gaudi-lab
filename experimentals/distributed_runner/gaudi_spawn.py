...
def parse_args():
    ...
    parser = ArgumentParser(
        description=(
            "Habana Gaudi distributed training launch helper utility that will \
                spawn up multiple distributed"
            " processes."
        )
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=29500,
        help="Master port used by DeepSpeed and MPI",
    )
...
def main():
    ...
    argv = [
        f'"{arg}"' if " " in arg and arg[0] != '"' and arg[-1] != '"' else arg
        for arg in sys.argv
    ]
